...
class PsfMeta3d(Meta):
    ...
    def add_para_xy(self, image, pos = (0, 0), rang = 20):
        if isinstance(image, Image_3d):
            ...
            x1, y1 = (x1 - pos[0]) / image.meta.unit_size[0], (y1 - pos[1]) / \
    ...
    def add_para_z(self, image, pos = (0, 0), rang = 20):
        if isinstance(image, Image_3d):
            ...
            z1 = (image.meta.meshgrid_1d() - pos[2]) / image.meta.unit_size[2]
...
def _gaussian_1d(sigz):
    ...
    return lambda z: 1 / _sqrt_pi / sigz * np.exp(-z ** 2 / 2 / sigz ** 2)


def _gaussian_2d(sigx, sigy):
    sigx = abs(sigx)
    sigy = abs(sigy)
    return lambda x, y: 1 / _sqrt_pi ** 2 / sigx / sigy * np.exp(-x ** 2 / 2 / sigx ** 2) * \
                        np.exp(-y ** 2 / 2 / sigy ** 2)
...
def _fitgaussian_2d(data, x1, y1):
    errorfunction = lambda p: np.ravel(_gaussian_2d(*p)(x1, y1) -
                                       data)
...
def _fitgaussian_1d(data, x):
    errorfunction = lambda p: np.ravel(_gaussian_1d(*p)(x) -
                                       data)
